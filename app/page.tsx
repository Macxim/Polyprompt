  ...
  return (
    <>
      ...
      <div className="flex flex-col min-h-screen">
        {/* Conditional Hero: Compact for returning users, Full for new users */}
        {state._hydrated && state.spaces.length > 0 ? (
          // Compact Dashboard Header for Returning Users
          <section className="bg-gradient-to-b from-slate-50/50 to-white border-b border-slate-100 py-8 px-8">
        ...
        {/* Capabilities Section - Only for new users */}
        {state._hydrated && state.spaces.length === 0 && (
        ...
        <section className="py-12 px-8 sm:px-0 max-w-6xl mx-auto w-full">
          {state._hydrated && state.spaces.length > 0 && (
            <>
              <div className="mb-8">
          ...
          {!state._hydrated && (
            <>
              <div className="mb-8">
                <div className="h-8 bg-slate-100/50 rounded-lg w-48 mb-2"></div>
                <div className="h-4 bg-slate-50/50 rounded-md w-64"></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="glass-card p-6 rounded-3xl border border-slate-200/60 shadow-sm animate-pulse h-[140px] bg-slate-50/20">
                    <div className="h-6 bg-slate-100/50 rounded-lg w-3/4 mb-4"></div>
                    <div className="space-y-2">
                      <div className="h-4 bg-slate-50/50 rounded-md w-1/2"></div>
                      <div className="h-4 bg-slate-50/50 rounded-md w-1/3"></div>
                    </div>
                  </div>
                ))}
              </div>
